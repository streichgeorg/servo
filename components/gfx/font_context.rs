...
use style_traits::values::font::{FontStyle, FontVariantCaps};
use webrender_api;
...
pub struct FontContext<T: 'static>
    where T: fmt::Debug + FontStyle + PartialEq
{
    ...
    where T: fmt::Debug + FontStyle + PartialEq
{
    ...
    pub fn layout_font_group_for_style(&mut self, style: ServoArc<T>)
                                       -> Rc<FontGroup> where T: fmt::Debug + FontStyle + PartialEq {
        ...
        let desc = FontTemplateDescriptor::new(style.get_font_weight(),
                                               style.get_font_stretch(),
                                               style.is_oblique_or_italic());

        let mut fonts: SmallVec<[Rc<RefCell<Font>>; 8]> = SmallVec::new();

        for family in style.get_font_families() {
                    ...
                    match cached_font_entry.font {
                        ...
                        Some(ref cached_font_ref) => {
                            ...
                            if cached_font.descriptor == desc &&
                               cached_font.requested_pt_size == style.get_size() &&
                               cached_font.variant == style.get_font_variant_caps() {
...
impl<T> Eq for LayoutFontGroupCacheKey<T> where T: fmt::Debug + FontStyle + PartialEq {}

impl<T> Hash for LayoutFontGroupCacheKey<T>
    where T: fmt::Debug + FontStyle + PartialEq
{
    fn hash<H>(&self, hasher: &mut H) where H: Hasher {
        self.pointer.get_hash().hash(hasher)
    }
}
